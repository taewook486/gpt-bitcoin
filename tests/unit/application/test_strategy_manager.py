...
def temp_instructions_dir(tmp_path: Path) -> Path:
    ...
    for strategy in ["conservative", "balanced"]:
        ...
        btc_strategy_file.write_text(
            f"# BTC {strategy.title()} Strategy\n\nBTC-specific {strategy} strategy.\n",
            encoding="utf-8",
        )
...
async def test_get_instruction_priority_chain(strategy_manager: StrategyManager):
    ...
    conservative_path = Path(strategy_manager.instructions_dir) / "current" / "conservative.md"
...
async def test_template_variable_substitution(
    ...
    assert "ETH-KRW" in content  # Correct ticker format: COIN-KRW
...
@pytest.mark.asyncio
async def test_cache_expiry(temp_instructions_dir: Path):
    """Test cache expiry and reload."""
    # Create manager with very short TTL (0.05 seconds)
    manager = StrategyManager(
        instructions_dir=str(temp_instructions_dir),
        cache_ttl_seconds=0.05,  # Very short TTL for testing
    )

    # First call - cache miss
    content1 = await manager.get_instruction(
        Cryptocurrency.BTC,
        TradingStrategy.CONSERVATIVE,
    )
    ...
    # Second call - cache should be expired, content reloaded
    content2 = await manager.get_instruction(
        Cryptocurrency.BTC,
        TradingStrategy.CONSERVATIVE,
    )

    # Content should be identical (same file)
    assert content1 == content2


@pytest.mark.asyncio
async def test_cache_invalidation_on_file_change(temp_instructions_dir: Path):
    ...
    # Modify the file
    btc_conservative_path = temp_instructions_dir / "coin_specific" / "BTC" / "conservative.md"
    time.sleep(0.1)  # Ensure time difference for mtime
    ...
    # Reload should detect file change and load new content
    content = await manager.get_instruction(
        Cryptocurrency.BTC,
        TradingStrategy.CONSERVATIVE,
    )
...
async def test_list_available_strategies(strategy_manager: StrategyManager):
    ...
    strategies = await strategy_manager.list_available_strategies(Cryptocurrency.BTC)
    ...
    strategies = await strategy_manager.list_available_strategies(Cryptocurrency.ETH)
...
async def test_get_strategy_metadata(strategy_manager: StrategyManager):
    ...
    metadata = await strategy_manager.get_strategy_metadata(TradingStrategy.CONSERVATIVE)
