...
class TestAlertManagerIntegration:
    ...
    def test_alertmanager_container_running(self, monitoring_stack):
        ...
        result = subprocess.run(
            ["docker", "ps", "--filter", "name=gpt-bitcoin-alertmanager", "--format", "{{.Status}}"],
            capture_output=True,
            text=True,
            timeout=10,
        )
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    pytest.fail(f"AlertManager health endpoint not accessible after {max_retries} attempts: {e}")
    ...
    def test_alertmanager_ui_accessible(self, monitoring_stack):
        ...
        assert response.status_code == 200, f"AlertManager UI not accessible: {response.status_code}"
    ...
    def test_prometheus_alertmanager_integration(self, monitoring_stack):
        ...
        assert response.status_code == 200, f"Failed to get Prometheus alertmanagers: {response.text}"
...
class TestAlertManagerEndToEnd:
    ...
    def test_alert_firing_and_resolution(self, monitoring_stack):
        ...
        response = requests.get("http://localhost:9093/api/v2/alerts", timeout=10)
        alerts = response.json()
        firing_alerts = [a for a in alerts if a["labels"].get("alertname") == "TestAlert" and a["status"]["state"] == "active"]
