        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    pytest.fail(f"Grafana health endpoint not accessible after {max_retries} retries")
...
class TestPrometheusDatasource:
    ...
    async def test_prometheus_query_via_grafana(
        ...
        query = {"queries": [{"refId": "A", "datasource": {"type": "prometheus", "uid": "prometheus"}, "expr": "up"}]}
...
class TestDashboardProvisioning:
    ...
    async def test_dashboard_panels_exist(
        ...
        async with http_session.get(
            ...
            expected_rows = ["Trading Performance", "API Metrics", "Cost Monitoring", "System Health"]
...
class TestGrafanaConfiguration:
    ...
    @pytest.mark.asyncio
    async def test_anonymous_access_disabled(
...
class TestSmokeTests:
    ...
    async def test_grafana_login(
        ...
        async with http_session.get(
            f"{GRAFANA_BASE_URL}/api/user", auth=grafana_auth
        ) as response:
            assert response.status == 200, f"Expected 200, got {response.status}"
            data = await response.json()
