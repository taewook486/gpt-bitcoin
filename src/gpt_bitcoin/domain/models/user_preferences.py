...
class UserPreferences:
    ...
    def _validate_allocation(self) -> None:
        ...
        if abs(total - 100.0) > 0.01:  # Allow small floating point errors
            raise ValueError(f"Portfolio allocation must sum to 100%, got {total:.2f}%")

    def get_coin_preference(self, coin: Cryptocurrency) -> CoinPreference | None:
        """
...
class UserPreferencesModel(BaseModel):
    ...
    def get_coin_preference(self, coin: Cryptocurrency) -> CoinPreferenceModel | None:
        """Get preference for a specific coin."""
        for pref in self.coins:
            if pref.coin == coin:
                return pref
