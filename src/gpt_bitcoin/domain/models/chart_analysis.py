...
class ChartAnalysis:
    ...
    def from_vision_response(cls, response: str) -> "ChartAnalysis":
        ...
        try:
            ...
            support_levels = [
                float(s) for s in data.get("support_levels", [])
            ]
            resistance_levels = [
                float(r) for r in data.get("resistance_levels", [])
            ]
...
class CombinedAnalysis:
    ...
    def __post_init__(self):
        ...
        if not 0.0 <= self.combined_confidence <= 1.0:
            raise ValueError(
                f"Combined confidence must be between 0.0 and 1.0, "
                f"got {self.combined_confidence}"
            )
