    ...
    def __init__(
        ...
        if self._config.enable_cache:
            if self._config.redis_url:
                ...
                logger.warning(
                    "Redis URL provided but Redis cache not implemented, "
    ...
    def select_optimal_model(
        ...
        # Medium complexity: balance based on token count
            ...
            if estimated_tokens < 500:
                selected_model = self.MODEL_GLM_4
                reasoning = (
                    "Medium complexity with small token count, "
                    "using cost-effective GLM-4"
                )
            else:
                selected_model = self.MODEL_GLM_5
                reasoning = (
                    "Medium complexity with larger token count, "
                    "using capable GLM-5 for quality"
                )
