...
class ChartGenerator:
    ...
    async def generate_chart(
        ...
        chart_path = (
            self.output_dir
            / f"{coin.value}_{period}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
        )
        ...
        # Note: mav expects periods (ints), not column names
        # Using pre-calculated MA columns instead of mav parameter
        fig, axes = mpf.plot(
            df,
            type="candle",
            style=s,
