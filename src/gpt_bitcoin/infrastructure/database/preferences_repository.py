...
class UserPreferencesRepository(ABC):
    ...
    @abstractmethod
    async def update_coin_strategy(self, coin: Cryptocurrency, strategy: TradingStrategy) -> None:
        """
...
class SQLiteUserPreferencesRepository(UserPreferencesRepository):
    ...
    async def _run_migrations(self) -> None:
        ...
        migration_path = Path(__file__).parent / "migrations" / "001_add_preferences.sql"
    ...
    async def _get_cached_preferences(self) -> UserPreferences | None:
        """
    ...
    async def get_preferences(self) -> UserPreferences:
        ...
        try:
            ...
            cursor = await conn.execute("SELECT * FROM user_preferences LIMIT 1")
            ...
            cursor = await conn.execute("SELECT * FROM coin_preferences")
