...
class TradingMetrics:
    ...
    def observe_trading_decision_duration(
        ...
        if self._config.enable_trading_metrics and self._trading_duration_histogram:
            self._trading_duration_histogram.labels(decision=decision).observe(duration_seconds)
            logger.debug(f"Observed trading decision duration: {duration_seconds}s for {decision}")
