"""
GPT Bitcoin Auto-Trading System - Streamlit Web UI
...
st.set_page_config(
    page_title="GPT Bitcoin Trading",
    page_icon="₿",
    layout="wide",
    initial_sidebar_state="expanded",
)

st.title("🤖 GPT Bitcoin Auto-Trading System")
...
def get_market_data(ticker: str, interval: str = "minutes/5", count: int = 100, testnet_mode: bool = False):
    ...
    async def _fetch():
        ...
        async with client:
            ohlcv = await client.get_ohlcv(ticker=ticker, interval=interval, count=count) if not testnet_mode else []
...
def get_ai_recommendation(ticker: str, strategy: TradingStrategy, instruction_version: str = "v3", testnet_mode: bool = False):
...
with tab1:
    st.subheader(f"실시간 시세 차트 - {ticker}")

    # Fetch market data
    with st.spinner("시장 데이터를 가져오는 중..."):
        try:
            ohlcv_data, current_price = get_market_data(
                ticker, interval="minutes/5", count=100, testnet_mode=st.session_state.testnet_mode
            )
        except Exception as e:
...
# CSV export and refresh buttons (outside if-else)
st.markdown("---")
    ...
    with col1:
        if st.button("📥 CSV 내보내기"):
            try:
                csv_data = asyncio.run(history_service.export_to_csv(trades))
                st.download_button(
                    label="다운로드",
                    data=csv_data,
                    file=f"trade_history_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                    mime="text/csv",
                )
            except Exception as e:
                st.error(f"CSV 내보내기에 실패했습니다: {e}")
...
st.markdown("---")
st.caption("GPT Bitcoin Auto-Trading System v5.0 | GLM-5/GLM-4.6V Powered")
