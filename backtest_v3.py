...
def print_results(df, trades, krw, btc, avg_buy_price, final_price, final_value):
    ...
    if btc > 0 and avg_buy_price > 0:
        ...
        print(f"  BTC 평균매수가 : {avg_buy_price:>17,.0f} KRW  (미실현 {'+' if unrealized >= 0 else ''}{unrealized:.1f}%)")
    ...
    print(f"\n{'─'*65}")
    print(f"  📌 Buy & Hold 비교 (전액 매수 후 보유)")
    print(f"{'─'*65}")
    ...
    print(f"  Alpha (초과수익): {'+' if alpha >= 0 else ''}{alpha:>14.2f}%  {'✅ 전략 우위' if alpha > 0 else '⚠ Buy&Hold 우위'}")
    ...
    print(f"  🔄 거래 통계")
    print(f"{'─'*65}")
    print(f"  총 거래 수     : {len(trades):>5}회 (매수 {len(buy_trades)}, 매도 {len(sell_trades)})")
    ...
    if sell_trades:
        ...
        if pnls:
            ...
            print(f"  승률           : {len(wins)}/{len(pnls)} ({len(wins)/len(pnls)*100:.0f}%)")
            if wins:
    ...
    if trades:
        ...
        for t in trades:
            ...
            print(f"  {str(t['date']):12} {t['action']:6} {t['price']:>18,.0f} {t['rsi']:>6.1f}  {pv:>14,.0f}")

    # 월별 가격 추이
    print(f"\n{'─'*65}")
    print(f"  📅 월별 BTC 가격 추이")
    print(f"{'─'*65}")
    ...
    for d, row in monthly.iterrows():
        ...
        sign = '+' if chg > 0 else ''
        print(f"  {d.strftime('%Y-%m')}  {row['first']:>16,.0f} → {row['last']:>16,.0f}  {sign}{chg:5.1f}%  {bar}")
    ...
    print(f"  SMA_10  : {latest['SMA_10']:>18,.0f}  {'↑ 종가 위' if latest['close'] > latest['SMA_10'] else '↓ 종가 아래'}")
    print(f"  EMA_10  : {latest['EMA_10']:>18,.0f}  {'↑ 종가 위' if latest['close'] > latest['EMA_10'] else '↓ 종가 아래'}")
    rsi_v = latest['RSI_14']
    ...
    print(f"  MACD    : {macd_v:>18.2f}  {'↑ 강세' if macd_v > macd_s else '↓ 약세'} (시그널 {macd_s:.2f})")
