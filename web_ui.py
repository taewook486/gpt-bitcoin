...
def get_market_data(
    ticker: str, interval: str = "minutes/5", count: int = 100, testnet_mode: bool = False
):
    ...
    async def _fetch():
        ...
        async with client:
            ohlcv = (
                await client.get_ohlcv(ticker=ticker, interval=interval, count=count)
                if not testnet_mode
                else []
            )
                    ...
                    with st.spinner("주문 검증 중..."):
                        try:
                            ...
                            approval = asyncio.run(
                                security_service.request_buy_order(
                            ...
                            if st.button("확인 및 주문 실행", type="primary", key="confirm_buy"):
                                with st.spinner("주문 실행 중..."):
                                    ...
                                    result = asyncio.run(
                                        security_service.execute_approved_trade(
                ...
                if (
                    ...
                    with st.spinner("주문 검증 중..."):
                        try:
                            ...
                            approval = asyncio.run(
                                security_service.request_sell_order(
        ...
        with col1:
            if st.button("🤖 AI 추천대로 자동 거래", type="primary", width="stretch"):
                ...
                try:
                    ...
                    krw_balance = asyncio.run(
                        get_account_info(testnet_mode=st.session_state.testnet_mode)
                    )
